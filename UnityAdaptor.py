...
from Entity import (ControlSignal, ROS2Point, State, WheelAngularVel,
                    WheelOrientation)
...
class UnityAdaptor:
    ...
    def calculate_relative_angle_unity_gloabl(
        self,
        lidar_direction_from_car_x,
        lidar_direction_from_car_y,
        car_orientation_degree_unity,
    ):
        relative_angle = self.angle_relative_to_x(
            -lidar_direction_from_car_y, lidar_direction_from_car_x
        )
    ...
    def transfer_obs(self, obs, unity_action):
        obs = json.loads(obs)
        ...
        car_quaternion = [
            obs["ROS2CarQuaternion"][0],
            obs["ROS2CarQuaternion"][1],
            obs["ROS2CarQuaternion"][2],
            obs["ROS2CarQuaternion"][3],
        ]
        ...
        wheel_quaternion_left_front = [
            obs["ROS2WheelQuaternionLeftFront"][0],
            obs["ROS2WheelQuaternionLeftFront"][1],
            obs["ROS2WheelQuaternionLeftFront"][2],
            obs["ROS2WheelQuaternionLeftFront"][3],
        ]  # 48 49 50 51ROS2WheelQuaternionRightBack
        (
            wheel_left_front_roll_x,
            wheel_left_front_pitch_y,
            wheel_left_front_yaw_z,
        ) = self.euler_from_quaternion(wheel_quaternion_left_front)

        wheel_quaternion_right_front = [
            obs["ROS2WheelQuaternionRightFront"][0],
            obs["ROS2WheelQuaternionRightFront"][1],
            obs["ROS2WheelQuaternionRightFront"][2],
            obs["ROS2WheelQuaternionRightFront"][3],
        ]
        (
            wheel_right_front_roll_x,
            wheel_right_front_pitch_y,
            wheel_right_front_yaw_z,
        ) = self.euler_from_quaternion(wheel_quaternion_right_front)
        # if len(obs['ROS2MinRange']) == 0:
        #     obs['ROS2MinRange'] = my_list = [0.0] * 180

        state = State(
            final_target_pos=ROS2Point(
                x=obs["ROS2TargetPosition"][0], y=obs["ROS2TargetPosition"][1], z=0.0
            ),  # obs[5]
            car_pos=ROS2Point(
                x=obs["ROS2CarPosition"][0], y=obs["ROS2CarPosition"][1], z=0.0
            ),  # obs[2]
            path_closest_pos=ROS2Point(
                x=obs["ROS2PathPositionClosest"][0],
                y=obs["ROS2PathPositionClosest"][1],
                z=0.0,
            ),  # obs[8]
            path_second_pos=ROS2Point(
                x=obs["ROS2PathPositionSecondClosest"][0],
                y=obs["ROS2PathPositionSecondClosest"][1],
                z=0.0,
            ),  # obs[11]
            path_farthest_pos=ROS2Point(
                x=obs["ROS2PathPositionFarthest"][0],
                y=obs["ROS2PathPositionFarthest"][1],
                z=0.0,
            ),  # obs[14] #obs[15~17] ROS2CarPosition
            car_vel=ROS2Point(
                x=obs["ROS2CarVelocity"][0], y=obs["ROS2CarVelocity"][1], z=0.0
            ),  # obs[20]
            car_orientation=car_orientation,
            wheel_orientation=WheelOrientation(
                left_front=self.radToPositiveRad(wheel_left_front_yaw_z),
                right_front=self.radToPositiveRad(wheel_right_front_yaw_z),
            ),
            car_angular_vel=obs["ROS2CarAugularVelocity"][2],  # obs[21 22]
            # obs[28]
            wheel_angular_vel=WheelAngularVel(
                left_back=obs["ROS2WheelAngularVelocityLeftBack"][1],  # obs[30]
                left_front=obs["ROS2WheelAngularVelocityLeftFront"][1],  # obs[31][33]
                right_back=obs["ROS2WheelAngularVelocityRightBack"][1],  # obs[34 36]
                right_front=obs["ROS2WheelAngularVelocityRightFront"][1],
            ),
            min_lidar=obs["ROS2Range"],  # 57 58 59
            min_lidar_direciton=obs["ROS2RangePosition"],
            action_wheel_angular_vel=WheelAngularVel(
                left_back=unity_action[0] / self.action_range,
                left_front=unity_action[1] / self.action_range,
                right_back=unity_action[2] / self.action_range,
                right_front=unity_action[3] / self.action_range,
            ),
            action_wheel_orientation=WheelOrientation(
                left_front=self.discritize_wheel_steering_angle(unity_action[0]),
                right_front=self.discritize_wheel_steering_angle(unity_action[0]),
            ),
        )
    ...
    def trasfer_action(self, ai_action):
        unity_action = [None] * len(ai_action)

        #### why cannot add negative?

        for i in range(len(ai_action)):
            unity_action[i] = ai_action[i] * self.action_range
            unity_action[i] = float( # type: ignore
                clamp(unity_action[i], -self.action_range, self.action_range)
            )

        # print(unity_action)
        action_sent_to_unity = [0.0, *[unity_action[i] for i in range(len(ai_action))]]
