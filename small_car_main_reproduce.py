...
from std_msgs.msg import ByteMultiArray, Float32MultiArray, String
...
class AiNode(Node):
    def __init__(self):
        ...
        self.subsvriber_ = self.create_subscription(
            String, "unity2Ros", self.receive_data_from_ros, 10
        )

        self.publisher_Ai2ros = self.create_publisher(
            Float32MultiArray, "ros2Unity", 10
        )  # Ai2ros #ros2Unity
...
class Env(Environment):
    ...
    def check_termination(self, state):
        ...
        self.distance_out = (
            distance >= self.end_distance[1] or distance <= self.end_distance[0]
        )
    ...
    def calculate_reward(self, state: Entity.State, new_state: Entity.State):
        ...
        prev_target_dist = self.calculate_distance(self.prev_pos, target_pos)
        ...
        if distance_diff > 0:
            ...
            dist_reward += 10 * distance_diff

        # # Collision penalty
        # min_lidar_dist = min(
        #     [
        #         new_state.min_lidar[0],
        #         (new_state.min_lidar[22] + new_state.min_lidar[23]) / 2,
        #         new_state.min_lidar[45],
        #         (new_state.min_lidar[67] + new_state.min_lidar[68]) / 2,
        #         new_state.min_lidar[90],
        #         (new_state.min_lidar[112] + new_state.min_lidar[113]) / 2,
        #         new_state.min_lidar[135],
        #         (new_state.min_lidar[157] + new_state.min_lidar[158]) / 2,
        #     ]
        # )
        # collision_penalty = 0
        # if min_lidar_dist < 0.5:
        #     print(f"Collision! min_lidar_dist: {min_lidar_dist}")
        #     collision_penalty = 0.5 - min_lidar_dist

        # print(
        #     f"dist_reward: {dist_reward} | angle_reward: {angle_reward} | collision_penalty: {collision_penalty}"
        # )
...
class Agt(Agent):
    ...
    def choose_actions(self, obs, prev_pos, trail_original_pos, inference):
        ...
        # noice
        if not inference:
            actions = actions + torch.tensor(self.noice(), dtype=torch.float).to(
                self.device
            )
    ...
    def processFeature(self, state: Entity.State, prev_pos, trail_original_pos):
        ...
        # feature.append((state.min_lidar[22] + state.min_lidar[23]) / 2)
        # feature.append(state.min_lidar[45])
        # feature.append((state.min_lidar[67] + state.min_lidar[68]) / 2)
        # feature.append(state.min_lidar[90])
        # feature.append((state.min_lidar[112] + state.min_lidar[113]) / 2)
        # feature.append(state.min_lidar[135])
        # feature.append((state.min_lidar[157] + state.min_lidar[158]) / 2)
...
def main(mode):
    ...
    agent = Agt(
        ...
        layer1_size=512,
        layer2_size=512,
        chpt_dir_load=chpt_dir_load,
        chpt_dir_save=chpt_dir_save,
    )
    ...
    state = Entity.State(
        final_target_pos=Entity.ROS2Point(x=0.0, y=0.0, z=0.0),
        ...
        car_angular_vel=0.0,
        wheel_angular_vel=Entity.WheelAngularVel(
            left_back=0.0, left_front=0.0, right_back=0.0, right_front=0.0
        ),
        min_lidar=[],
        ...
        action_wheel_angular_vel=Entity.WheelAngularVel(
            left_back=0.0, left_front=0.0, right_back=0.0, right_front=0.0
        ),
        action_wheel_orientation=Entity.WheelOrientation(
            left_front=0.0, right_front=0.0
        ),
    )
    ...
    #             print(
    #                 "test run: ", i, ",reward: %.2f," % (score), "ctr: ", env.game_ctr
    #             )
